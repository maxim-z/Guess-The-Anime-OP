...
function MediaPlayer({ hintsRevealed, videoId, showVideo, songTitle, songArtist } : MediaPlayerProps) {
    ...
    const parentVariants = { // parent doesn't need to animate anything its used for events for its children
        hidden: {}, show: {}, plusVol: {}, minusVol: {}, bumpReset: {}
    };
    const transition = { duration: 0.5 };
    const fastTransition = { duration: 0.25 };
    const minusVariants = {
        hidden: { opacity: 0 },
    ...
    const sliderVariants = {
        ...
        bumpReset: {} // only the +/- are affected
    };
    ...
    const minusVol = async () => {
        ...
        controlsVolume.set('bumpReset');
        await controlsVolume.start('minusVol');
        controlsVolume.start('hidden');
    };
    const plusVol = async () => {
        setVolume((prev) => {
    ...
    if (showVideo) { // when an end game state is reached show just the yt video player
        return (
    ...
    } else { // while the game is in a play state show the customized media player
        return (
            <div className="MediaContainer w-full h-[200px] md:h-[270px] flex items-center justify-center">
                ...
                <div className='PlayerContainer relative w-[350px] h-[200px] origin-center md:scale-135'>
                    ...
                    <div className="Media absolute w-[300px] h-[150px] bottom-0 left-[25px] 
                        ...
                        <div className="VolumeSlider absolute w-[20px] h-[35px] top-[15px] right-[-20px] rounded-r-lg bg-blue-500">
                            {[...Array(18)].map((_, i) => { // i==0,17 -> 16px width, i==1,16 -> 17px width, i==[2,...15] -> 18px width
                                ...
                                return (
                                    <div 
                                        key={`volknobgroove_${i}`}
                                        className='VolumeKnobGroove absolute h-[1px] bg-slate-900'
                                        style={{ 
                        ...
                        <div className='SlidersContainer relative w-full h-[30px] top-[-33px] z-1'>
                            <div className="PlayTimeBar relative flex w-full h-full">
                                <div 
                                    className='TimeShow relative w-[25px] h-[25px] left-1 text-lg'
                                    style={generateShadow(10)}
                                >
                                    {Math.min(Math.floor(currentTime), interval[hintsRevealed])}
                                </div>
                                <input 
